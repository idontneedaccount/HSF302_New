...
export class RoomdetailsComponent {
  ...
  calculateTotalPrice():number {
    ...
    return this.room?.price * totalDays || 0;
  }
  ...
  acceptBooking():void{
    ...
    this.apiService.bookRoom(booking).subscribe({
      next: (res: any) =>{
        if (res.status === 200) {
          // Check if booking reference is available
          if (res.bookingReference) {
            // Navigate to payment page with booking reference and amount as path parameters
            this.router.navigate(['/payment', res.bookingReference, this.totalPrice]);
          } else {
            // Fallback to old behavior if no booking reference
            this.message = "Your Booking is Successful. An Email of your booking details and the payment link has been sent to you";
            setTimeout(()=>{
              this.message = null;
